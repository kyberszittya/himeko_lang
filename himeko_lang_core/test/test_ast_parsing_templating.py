...
class TestAstParsingTemplating(TestAncestorTestCase):
    ...
    def test_basic_templating(self):
        ...
        self.assertEqual(len(n_0_0.stereotype), 0)
        ...
        # Check template
        self.assertIn("node0", n_2.stereotype)
        # Check template parent
        self.assertEqual("node_lev_0", n_2.stereotype[0].parent.name, )
        # Check node3 of node_lev_0
        n_3 = next(n_lev_0.get_children(lambda x: isinstance(x, HyperVertex) and x.name == "node3", None))
        # Check template
        self.assertEqual(n_3.stereotype[0].name, "node2")
        # Check template parent
        self.assertEqual(n_3.stereotype[0].parent.name, "node_lev_0")
        ...
        self.assertEqual(len(n_0_1.stereotype), 0)
